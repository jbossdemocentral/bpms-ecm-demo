...
RUN mvn clean install -f /opt/jboss/bpms-projects/brms-file-upload-cmis/pom.xml \
  && cp -r /opt/jboss/bpms-projects/brms-file-upload-cmis/target/brms-file-upload-cmis-1.0.0.jar  $BPMS_HOME/standalone/deployments/business-central.war/WEB-INF/lib \
  && chown -R jboss:jboss $BPMS_HOME/standalone/deployments/business-central.war/WEB-INF/lib $BPMS_HOME/bin/.niogit $BPMS_HOME/standalone/configuration/application-roles.properties $BPMS_HOME/standalone/configuration/standalone.xml $BPMS_HOME/standalone/deployments/business-central.war/WEB-INF/classes/userinfo.properties \
