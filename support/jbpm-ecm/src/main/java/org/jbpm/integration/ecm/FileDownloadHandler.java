...
public class FileDownloadHandler extends BeanHandler {
	...
	private static final long serialVersionUID = -3973943127227839622L;
    ...
    public CommandResponse actionDownload(CommandRequest request) {
        try {
            ...
            if (!StringUtils.isEmpty(content)) {
                ...
                File file = fileStorageService.getFile(idElements[1]);
                if (file.exists()) { 
                	return new SendStreamResponse(new FileInputStream(file), "inline;filename=" + file.getName());
                }
            }
        } catch (Exception e) {
